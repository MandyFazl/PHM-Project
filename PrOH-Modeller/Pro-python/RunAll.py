...
def process_script1(filename_with_identifier, combined_presentation):
    try:
 ...
 # Non-Core-Process-Statment       
def process_script2(filename_with_identifier, combined_presentation):
    try:
...
# Decision_Bubbles.py
def process_script3(filename_with_identifier, combined_presentation):
    try:
        logger.info("Processing script3")
...
#SUb-bubbles-Bracket.py
def process_script4(filename_with_identifier, combined_presentation):
    try:
        ...
        with open(combined_presentation, 'a', newline='') as csv_output:
            writer = csv.writer(csv_output)
            writer.writerow(['Words Between Parentheses'])  # Header
            writer.writerows([[word] for word in parentheses_words])

        logger.info(f"Words between parentheses have been saved to '{combined_presentation}'")

        # Call the Bracket-Sketch-unique.py script with the output of this script as its input
        subprocess.run(['python', 'Bracket-Sketch-unique.py', combined_presentation])
