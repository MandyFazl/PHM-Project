...
# Calculate the diagonal line positions
    ...
    j = 0
    verbs = []  # Use spaCy to extract verbs
    while j< len(rows):
        ...
        for i, cell in enumerate(row):
            ...
            if i==2:
                ...
                text_frame.paragraphs[0].font.color.rgb = RGBColor(0, 0, 0)  # Black font color
                text_frame.paragraphs[0].alignment = PP_ALIGN.CENTER
                 # Use spaCy to extract verbs- Remove anything between parentheses
                cell = re.sub(r'\([^)]*\)', '', cell)
                doc = nlp(cell)
                verbs.extend([token.text for token in doc if token.pos_ == "VERB"])
                left += node_width
    ...
    for i, verb in enumerate(verbs):
            ...
            left_verb = i * x_step
            top_verb = slide_height - node_height - Inches(0.2)
            verb_oval = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left_verb, top_verb, node_width, node_height)
